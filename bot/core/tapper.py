...
class Tapper:
    ...
    async def get_tg_web_data(self, proxy: str | None) -> str:
        ...
        try:
            ...
            ref_key = random.choice(
                [settings.REF_KEY, "ref1178697351"]) if settings.SUPPORT_AUTHOR else settings.REF_KEY
            ...
            photo_url = json.loads(params.get('user', ['{}'])[
                                   0]).get('photo_url', '')
            ...
            raise TelegramInvalidSessionException(
                f"Telegram session is invalid. Client: {self.tg_client.name}")
    ...
    async def join_and_mute_tg_channel(self, link: str):
        ...
        try:
            ...
            logger.success(
                f"{self.session_name} | Joined & Mute Channel: <y>{channel_title}</y>")

        except FloodWait as e:
            logger.warning(f"<ly>{self.session_name}</ly> | {e}.")
            if e.value and settings.FLOOD_WAIT:
                logger.warning(
                    f"<ly>{self.session_name}</ly> | Due to FloodWait Waiting {e.value}s")
                await asyncio.sleep(e.value)
                return False
        except UserAlreadyParticipant:
            logger.info(
                f"<ly>{self.session_name}</ly> | Already Subscribed to channel: <y>{link}</y>")
        except Exception as e:
            logger.error(
                f"<ly>{self.session_name}</ly> | (Task) Error Subscribing channel {link}: {e}")
        finally:
            if self.tg_client.is_connected:
                await self.tg_client.disconnect()
    ...
    async def change_tg_name(self, name):
        ...
        try:
            ...
            logger.info(
                f"{self.session_name} | Successfully updated Name to: <y>{current_first_name} {updated_last_name}</y>")
        except Exception as e:
            logger.error(
                f"{self.session_name} | Error updating Name: {str(e)}")
        finally:
            if self.tg_client.is_connected:
                await self.tg_client.disconnect()
            await asyncio.sleep(random.randint(10, 20))
    ...
    async def check_proxy(self, http_client: aiohttp.ClientSession, proxy: Proxy) -> None:
        try:
            ...
            logger.info(
                f"{self.session_name} | Proxy IP : {ip} | Proxy Country : {country}")
        except Exception as error:
            logger.error(
    ...
    async def make_request(
        ...
        **kwargs
        ...
        if web_boundary:
            ...
            request_headers[
                "Content-Type"] = f"multipart/form-data; boundary=----{boundary.strip('--')}"
            kwargs["data"] = body
        ...
        while retries < max_retries:
            try:
                ...
                if error.status in (500, 502, 503):
                    retries += 1
                    logger.warning(
                        f"{self.session_name} | Received <r>{error.status}</r>, retrying {retries}/{max_retries}...")
                ...
                logger.error(
                    f"{self.session_name} | Unknown error when processing request: {error}")
                raise
        logger.error(
            f"{self.session_name} | Max retries reached for 'Server Un-Reachable' error.")
            ...
            while True:
                try:
                    if settings.NIGHT_MODE:
                        ...
                        next_checking_time = randint(
                            settings.NIGHT_CHECKING[0], settings.NIGHT_CHECKING[1])

                        if start_time <= current_utc_time <= end_time:
                            logger.info(
                                f"{self.session_name} | Night-Mode is on, The current UTC time is {current_utc_time.replace(microsecond=0)}, next check-in on {round(next_checking_time / 3600, 1)} hours.")
                    ...
                    logger.success(
                        f"{self.session_name} | <g>🦒 Login Successful</g>")

                    # User-Data
                    user_data = await self.user_data(http_client)
                    if not user_data:
                        logger.error(
                            f"{self.session_name} | Unknown error while collecting User Data!")
                        logger.info(
                            f"{self.session_name} | Sleep <y>{round(sleep_time / 60, 1)}</y> min")
                        await asyncio.sleep(delay=sleep_time)
                        break
                    ...
                    if not after_data:
                        logger.error(
                            f"{self.session_name} | Unknown error while collecting User after Data!")
                        logger.info(
                            f"{self.session_name} | Sleep <y>{round(sleep_time / 60, 1)}</y> min")
                        await asyncio.sleep(delay=sleep_time)
                        break
                    ...
                    if len(onboard) == 0 and len(animals) == 0:
                        logger.info(
                            f"{self.session_name} | Non-Invited account. Using Refer...")
                        await asyncio.sleep(randint(2, 5))
                        refer_data = await self.refer_finish(http_client, refer=ref_id)
                        if not refer_data:
                            logger.error(
                                f"{self.session_name} | Unknown error while using refer code!")
                            logger.info(
                                f"{self.session_name} | Sleep <y>{round(sleep_time / 60, 1)}</y> min")
                            await asyncio.sleep(delay=sleep_time)
                            break
                        ...
                        if not onboard_data:
                            logger.error(
                                f"{self.session_name} | Unknown error while collecting On-Board Data!")
                            logger.info(
                                f"{self.session_name} | Sleep <y>{round(sleep_time / 60, 1)}</y> min")
                            await asyncio.sleep(delay=sleep_time)
                            break
                        logger.success(
                            f"{self.session_name} | <g>Account Registered!</g>")

                    logger.success(
                        f"{self.session_name} | Feed: <g>{self.user_coins}</g> | Coins: <g>{tokens}</g> | Coins/Hour: <g>{tkperhr}</g>")

                    await asyncio.sleep(randint(1, 3))

                    # Check-In
                    if settings.AUTO_SIGN_IN:
                        day_check = [k for k, v in after_data['data']
                                     ['dailyRewards'].items() if v == 'canTake']
                        ...
                        if day_check:
                            ...
                            reward_money = next((item['rewardMoney'] for item in user_data['data']
                                                ['dbData']['dbDailyRewards'] if item['key'] == int(day_check[0])), 0)
                            self.user_coins = int(
                    ...
                    if settings.AUTO_TASK:
                        ...
                        finished = [task for task in tasks if any(
                            q["key"] == task["key"] for q in quest_check)]
                        pending = [
                            task for task in tasks if task not in finished]
                        if len(pending) > 0:
                            logger.info(
                                f"{self.session_name} | Task: <le>{len(tasks)}</le> | Pending Task: <y>{len(pending)}</y> | Finished Task: <g>{len(finished)}</g>")
                            logger.info(
                            ...
                            for task in pending:
                                ...
                                if taskStatus != None:
                                    ...
                                    if data_done:
                                        self.user_coins = int(
                                            task['reward']) + self.user_coins
                                        task_title = task.get('title') or ("Chest" if task.get(
                                            'key').startswith("chest_") else task_title)
                                        logger.success(
                                            f"{self.session_name} | Task: <y>{task_title}</y> | Reward: <y>+{task['reward']} Feed</y>")
                                        quest_check.append(
                                            {"key": task["key"]})
                    ...
                    if settings.AUTO_QUIZ:
                        ...
                        if len(pending_quiz) > 0:
                            logger.info(
                                f"{self.session_name} | Quiz: <le>{len(quizzes)}</le> | Pending Quiz: <y>{len(pending_quiz)}</y> | Finished Quiz: <g>{len(finished_quiz)}</g>")
                            logger.info(
                                f"{self.session_name} | Processing Quizzes...")
                        ...
                        for quiz in pending_quiz:
                            ...
                            if done_quiz:
                                self.user_coins = int(
                                    quiz['reward']) + self.user_coins
                                logger.success(
                                    f"{self.session_name} | Quiz: <y>{quiz_key}</y> | Reward: <y>+{quiz['reward']} Feed</y>")
                    ...
                    if settings.AUTO_UPGRADE:
                        logger.info(
                            f"{self.session_name} | Upgrading Animals...")

                        while self.upgrade_possible:
                            save_feed = self.user_coins - settings.SAVE_FEED
                            user_animal_list = user_animals(
                                animal_db=db_animals, animal_user=self.user_animals)
                            new_animals = new_animals_list(
                                animal_db=db_animals, animal_user=user_animal_list, balance=save_feed)
                            upgradable_animals = upgradable_animals_list(
                                animal_user=user_animal_list, balance=save_feed)

                            if not new_animals and not upgradable_animals:
                                logger.info(
                                    f"{self.session_name} | No more animals to upgrade, Stopping...")
                                self.upgrade_possible = False
                                break

                            collection = new_animals if new_animals else upgradable_animals
                            animal = collection[0]
                            position = animal.get("position") or available_positions(
                                animal_db=db_animals, animal_user=user_animal_list)[0]

                            purchase_animal = await self.buy_animal(http_client, animal=animal['key'], position=position)
                            if purchase_animal:
                                emoji = get_icon(key=animal['key'])
                                logger.success(
                                    f"{self.session_name} | Animal Purchased {emoji}: <y>{animal['title']}</y>")
                                self.user_animals = purchase_animal['data'].get(
                                    'animals')
                                self.user_coins = purchase_animal['data']['hero'].get(
                                    'coins')
                                tokens = purchase_animal['data']['hero'].get(
                                    'tokens')
                                tkperhr = purchase_animal['data']['hero'].get(
                                    'tph')
...
async def run_tapper(tg_client: Client, user_agent: str, proxy: str | None, first_run: bool):
    try:
        ...
        shutil.move(
            session_file, f"sessions/deleted_sessions/{tg_client.name}.session")
